use aws_lambda_events::event::{sqs::SqsMessage, s3::S3Event};
use tracing::{debug, info};
use anyhow::{Context, Result};
use serde_json;

/// Business logic for processing S3 events from SQS messages
pub async fn process_s3_event_message(sqs_message: &SqsMessage) -> Result<()> {
    // Parse S3 event from SQS message body
    let body = sqs_message.body.as_ref()
        .context("SQS message has no body")?;
    
    let s3_event: S3Event = serde_json::from_str(body)
        .context("Failed to parse S3 event from SQS body")?;
    
    info!("Processing S3 event with {} records", s3_event.records.len());
    
    for record in s3_event.records {
        let bucket = record.s3.bucket.name.as_ref()
            .context("S3 record missing bucket name")?;
        let key = record.s3.object.key.as_ref()
            .context("S3 record missing object key")?;
        let event_name = record.event_name.as_ref()
